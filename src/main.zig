...
pub fn main() !void {
    ...
    std.log.debug("args: {s}, {d}\n", .{ args[1], args.len });

    var input_file: []u8 = undefined;
    if (args.len > 1) {
        input_file = args[1];
    } else {
        std.debug.print("{s}\n", .{"Usage: ./hex2bin-fork xxx.hex"});
        return;
    }
    ...
    while (try in_stream.readUntilDelimiterOrEof(&buf, '\n')) |line| {
        switch (line[8]) {
            @intFromEnum(ReacType.data) => {
                ...
                std.log.debug("addr:{x}, {X}\n", .{ std.fmt.fmtSliceHexLower(addr_list), std.fmt.fmtSliceEscapeUpper(bin_buf) });
            },
            @intFromEnum(ReacType.end_of_file) => {
                std.log.debug("{s}\n", .{"end file"});
            },
            @intFromEnum(ReacType.ext_seg_addr) => {
            ...
            @intFromEnum(ReacType.start_seg_addr) => {
                std.log.debug("start_seg_addr: {s}\n", .{line[9 .. line.len - 3]});
            },
            @intFromEnum(ReacType.ext_linear_addr) => {
                var tmp: u32 = try parseU32(line[9 .. line.len - 3]);
                addr_offset = tmp << 16;
